...
func TestHandleListContainers(t *testing.T) {
    t.Run("returns trimmed container list", func(t *testing.T) {
        fake := &fakeDockerClient{
            containerList: client.ContainerListResult{ /* Items with known IDs/Names */ },
        }
        s := newTestService(fake)

        // Build the handler via the mux.
        ...
        // Assert against whatever you put in containerList.Items.
    })
