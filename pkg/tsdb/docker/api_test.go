...
func TestGetContainerStats(t *testing.T) {
    ...
    t.Run("malformed JSON returns error", func(t *testing.T) {
        body := `{"cpu_stats":{"cpu_usage":{"total_usage":1000000},"online_cpus":4}}`
		fake := &fakeDockerClient{
			statsResult: client.ContainerStatsResult{
				Body: io.NopCloser(strings.NewReader(body)),
			},
...
func TestGetSystemDF(t *testing.T) {
    ...
    // Happy-path test depends on the exact DiskUsageResult shape; fill in once
    // you know the real type. Construct a DiskUsageResult with known counts and
    // assert the resulting SystemDF fields match.
}
